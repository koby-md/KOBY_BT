...
END:VCARD
        ...
        reply: {
            value(jid, text = '', quoted, options) {
                const isLid = jid && jid.endsWith('@lid') // 🟢 Fix: إزالة الاقتباس بالنسبة للحسابات المخفية
                return Buffer.isBuffer(text) 
                    ? conn.sendFile(jid, text, 'file', '', isLid ? null : quoted, false, options) 
                    : conn.sendMessage(jid, { ...options, text }, { quoted: isLid ? null : quoted, ...options })
            }
        },
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
            get() {
                ...
                if (!jid) return jid
                return jid.decodeJid() // 🟢 Fix 1: هاد التعديل غيخلي المعرف صحيح ومغيحيدش الدومين @lid
            },
            enumerable: true
        },
        ...
        reply: {
            async value(text, chatId, options = {}) {
                ...
                const isLid = jid.endsWith('@lid') // 🟢 Fix 2: إزالة الرد المقتبس للحسابات المخفية باش واتساب يخليه يدوز

                return await this.conn.sendMessage(jid, {
                    text: text,
                    ...options
                }, {
                    quoted: isLid ? null : this
                })
            }
        },
        copy: {
